...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """ updates beliefs based on the emission model and previous beliefs """
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """ iterates over the legalpositions and adds positions from them, the distribution
        is even"""
        self.particles=[]
        self.beliefs=util.Counter()
        for i in range(self.numParticles):
            pos=i%len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        """ updates beliefs after an event observation"""
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        if noisyDistance==None:
    ...
    def elapseTime(self, gameState):
        ...
        """ updates the particles as the time passes """
        newList=list()
    ...
    def getBeliefDistribution(self):
        ...
        """ generates beliefsbased on the current particles """
        currentWeights=util.Counter()
        for particle in self.particles:
            currentWeights[particle]+=1.0/float(self.numParticles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """ gets cartesian product of the positions from the legal positions and 
        then shuffles it and adds tuples in the particles one by one"""
        permuts=list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(permuts)
        self.particles=[]
        for i in range(self.numParticles):
            self.particles.append(permuts[i%len(permuts)])
    ...
    def observeState(self, gameState):
        ...
        """ updates beliefs based on the evidence"""
        pacmanPosition = gameState.getPacmanPosition()
        ...
        if probs.totalCount()==0:
            self.initializeParticles()
            
        else:
            self.particles=[]
            for i in range(self.numParticles):
                self.particles.append(util.sample(probs))
            for i in range(self.numGhosts):
                if noisyDistances[i]==None:
                    newList=[]
                    for particle in self.particles:
                        newList.append(self.getParticleWithGhostInJail(particle, i))
                    self.particles=newList
    ...
    def elapseTime(self, gameState):
        ...
        """ updates beliefs as the time elapses"""
        newParticles = []
    ...
    def getBeliefDistribution(self):
        ...
        """ generates beliefs based on the particles """
        count=util.Counter()
        for particle in self.particles:
            count[particle]+=1.0
        count.normalize()
