...
from samples import DATUM_WIDTH, DATUM_HEIGHT
...
def enhancedFeatureExtractorDigit(datum):
    ...
    "just counting the number of connected components, and i have three binry features indicating the number of connected components"""
    
    
    visited=[]
    result=0
    datum=datum.getPixels()
    for i in range(DIGIT_DATUM_WIDTH):
        for j in range(DIGIT_DATUM_HEIGHT):
            if datum[i][j]==0 and (i, j) not in visited:
                ...
                fringe.append(cur)
                while(True):
                    """ if the fringe is empty then the current connected component is ended"""
                    if len(fringe)==0:
                        break
                    current=fringe[0]
                    """ the next 4 if statements check whether the neighbour pixel should be considered or not"""
                    if(datum[current[0]][current[1]]==1 or datum[current[0]][current[1]]==2):
                        fringe.remove(current)
                        continue
                    if current[0]+1<DIGIT_DATUM_WIDTH:
                    ...
                    if current[1]+1<DIGIT_DATUM_HEIGHT:
                        if (current[0], current[1]+1) not in visited:
                            fringe.append((current[0], current[1]+1))
                            visited.append((current[0], current[1]+1)) 
                    fringe.remove(current) 
...
def enhancedPacmanFeatures(state, action):
    ...
    """ for features I used minimal distance to the ghosts, the number of foods, minimal distance to the capsules and minimal distance to the food """
    
    
    newstate=state.generateSuccessor(0, action)
    
    ghosts=newstate.getGhostPositions()
    pacmanPos=newstate.getPacmanPosition()
    dist=10000000000
    for pos in ghosts:
        dist=min(manhattanDistance(pos, pacmanPos), dist)
    features['ghostMinDist']=-1.0/(dist+1)
    foodsNum=newstate.getNumFood()
    features['foodNum']=1.0/(foodsNum+1)
    capsules=newstate.getCapsules()
    capsDist=10000000000
    for pos in capsules:
        capsDist=min(capsDist, manhattanDistance(pacmanPos, pos))
    features['capsuleDistance']=1.0/(capsDist+1)
    food=newstate.getFood().asList()
    foodDist=100000000000
    for pos in food:
        foodDist=min(foodDist, manhattanDistance(pos, pacmanPos))
    features['foodDist']=1.0/(foodDist+1)
    return features
